...
```text
智慧社区智能监控视频采集与推流系统（RK3566）
```

项目描述：

```text
面向智慧社区智能监控场景，基于 RK3566 和 IMX415 摄像头构建端侧视频链路，实现 Sensor 采集、V4L2 取帧、YOLO11n RKNN 推理、H.264 硬件编码与 RTSP 推流，完成从摄像头到网络侧预览的稳定输出。
```

个人职责：

```text
负责 RK3566 端侧开发，完成 IMX415 驱动适配、V4L2 采集链路、RGA 预处理、YOLO11n RKNN 部署、MPP H.264 编码和 RTSP 推流；通过分阶段打点定位采集、推理、编码和推流耗时瓶颈，并进行低延迟优化。
```

优化成果：

```text
将 V4L2 buffer 数量由 4 调整为 2，降低旧帧排队；基于 YOLO 推理降频与结果复用降低 NPU 平均负载；优化 MPP 编码参数并去除冗余 memcpy；将串行链路改造为 V4L2 采集线程与处理/编码/推流线程分离的低延迟流水线，并进一步采用 V4L2 buffer 直通交接策略，将端到端延迟从约 213-216 ms 降低到约 71 ms。
```

AI 部署表述：

```text
基于 RK 官方 YOLO11n INT8 RKNN 模型完成端侧部署，使用 RGA 完成 NV12 到 RGB888 的 letterbox 预处理，并通过 RKNN 推理细分打点定位模型推理瓶颈；进一步检查 NPU devfreq，发现默认 600 MHz 满载运行，理论满频后可接近官方 benchmark。
```
...
- 更换 5V3A 电源。
- 固定 NPU performance governor 后复测 YOLO 推理。
- 开启 RKNN profiling 获取 layer 级耗时。
- 判断是否存在 CPU fallback。

中期：

- 尝试 YOLO11n 416x416 / 320x320 输入尺寸。
- 评估 YOLOv5n / YOLOv6n 在 RK3566 上的性能。
- 使用真实监控图像重新做 INT8 calibration。

长期：

- 实现 RGA blit 到编码 DMA-BUF。
- MPP 直接导入编码 DMA-BUF。
- 将检测框叠加从 CPU 逐像素绘制迁移到 RGA。
- 接入服务端转发和客户端预览，形成完整监控系统。
