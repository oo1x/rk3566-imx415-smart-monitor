...
typedef enum {
    ...
    SUBMIT_LATEST_PENDING = 3,
} submit_policy_t;

static submit_policy_t g_submit_policy = SUBMIT_LATEST_PENDING;
...
typedef struct {
    ...
    int64_t pending_capture_ts_us;
    int64_t pending_submit_ts_us;
...
static void swap_nv12_ptr(unsigned char **a, unsigned char **b)
{
    unsigned char *tmp = *a;
    *a = *b;
    *b = tmp;
                         ...
                         int width, int height)
{
    ...
    ctx->pending_buf = (unsigned char *)malloc(ctx->frame_size);
    ctx->scratch_buf = (unsigned char *)malloc(ctx->frame_size);
    if (ctx->queue_bufs) {
    ...
    int allocation_ok = ctx->queue_bufs && ctx->queue_seq &&
                        ...
                        ctx->work_buf && ctx->pending_buf && ctx->scratch_buf;
                           ...
                           int64_t submit_ts_us, int *queue_depth)
{
    ...
    if (g_submit_policy == SUBMIT_LATEST_PENDING) {
        if (!ctx->scratch_buf || !ctx->pending_buf || !queue_depth)
            return 0;
        memcpy(ctx->scratch_buf, nv12, ctx->frame_size);
        pthread_mutex_lock(&ctx->mutex);
        int replaced = ctx->pending_valid;
        swap_nv12_ptr(&ctx->scratch_buf, &ctx->pending_buf);
        ctx->pending_seq = seq;
        ctx->pending_capture_ts_us = capture_ts_us;
        ctx->pending_submit_ts_us = submit_ts_us;
        ctx->pending_valid = 1;
        if (replaced)
            ctx->replaced_count++;
        *queue_depth = 1;
...
static int ai_async_queue_depth(ai_async_ctx_t *ctx)
{
    int depth;
    pthread_mutex_lock(&ctx->mutex);
    depth = (g_submit_policy == SUBMIT_LATEST_PENDING) ?
            ctx->pending_valid : ctx->queue_count;
    pthread_mutex_unlock(&ctx->mutex);
    return depth;
}
...
static void maybe_write_queue_status(ai_async_ctx_t *ctx, uint64_t frame_id, int force)
{
    ...
    pthread_mutex_lock(&ctx->mutex);
    int queue_depth = (g_submit_policy == SUBMIT_LATEST_PENDING) ?
                      ctx->pending_valid : ctx->queue_count;
    int busy = ctx->busy;
    uint64_t submitted = ctx->submitted_count;
    uint64_t skipped = ctx->skipped_count;
    uint64_t replaced = ctx->replaced_count;
    ...
    if (fp) {
        fprintf(fp,
                ...
                "infer_replaced=%llu infer_completed=%llu\n",
                (long long)timestamp, (unsigned long long)frame_id,
                queue_depth, busy, (unsigned long long)submitted,
                (unsigned long long)skipped, (unsigned long long)replaced,
...
static void *ai_thread_main(void *opaque)
{
    ...
    while (g_running) {
        ...
        pthread_mutex_lock(&ctx->mutex);
        if (g_submit_policy == SUBMIT_LATEST_PENDING) {
            while (g_running && !ctx->pending_valid)
            ...
            tmp = ctx->work_buf;
            ctx->work_buf = ctx->pending_buf;
            ctx->pending_buf = tmp;
            seq = ctx->pending_seq;
            capture_ts_us = ctx->pending_capture_ts_us;
            submit_ts_us = ctx->pending_submit_ts_us;
            ctx->pending_valid = 0;
            ctx->busy = 1;
            ctx->submitted_count++;
        } else {
        ...
        ctx->busy = 0;
        ctx->completed_count++;
        int pending_depth = (g_submit_policy == SUBMIT_LATEST_PENDING) ?
                            ctx->pending_valid : ctx->queue_count;
        pthread_mutex_unlock(&ctx->mutex);
...
static void *process_thread_main(void *opaque)
{
    ...
    while (g_running) {
        ...
        if (!g_ai_enabled || g_submit_policy == SUBMIT_DISABLED)
            ...
            decision_reason = "pending_replaced";
        else if (submit_result == -1)
...
int main(int argc, char *argv[])
{
    ...
    if (policy_env) {
        ...
        else if (strcmp(policy_env, "latest_pending") == 0)
