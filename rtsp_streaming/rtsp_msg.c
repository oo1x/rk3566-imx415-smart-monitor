...
static int rtsp_msg_parse_auth (rtsp_msg_s *msg, const char *line)
{
	...
	p++;
	while (*p == ' ') p++;
	if (0 == strncmp(p, "Basic ", 6)) {
		hdrs->auth->type = RTSP_MSG_AUTH_TYPE_BASIC;
		p += 6;
		while (isgraph(*p) && tmp < sizeof(hdrs->auth->basic_b64) - 1) {
			hdrs->auth->basic_b64[tmp++] = *p++;
		}
		hdrs->auth->basic_b64[tmp] = 0;
	} else if (0 == strncmp(p, "Digest ", 7)) {
...
static int rtsp_msg_build_auth (const rtsp_msg_s *msg, char *line, int size)
{
	return 0;//TODO
}
...
static int rtsp_msg_build_www_auth (const rtsp_msg_s *msg, char *line, int size)
{
	if (msg->hdrs.www_auth) {
		if (msg->hdrs.www_auth->type == RTSP_MSG_AUTH_TYPE_BASIC) {
			snprintf(line, size, "WWW-Authenticate: Basic realm=\"%s\"\r\n", 
...
//like CSeq/Session int
#define DEFINE_PARSE_BUILD_LIKE_CSEQ(_name, _type, _param, _fmt) \
...
DEFINE_PARSE_BUILD_LIKE_CSEQ(cseq, rtsp_msg_cseq_s, cseq, "CSeq: %u")
DEFINE_PARSE_BUILD_LIKE_CSEQ(session, rtsp_msg_session_s, session, "Session: %08X")
DEFINE_PARSE_BUILD_LIKE_CSEQ(content_length, rtsp_msg_content_length_s, length, "Content-Length: %u")
DEFINE_PARSE_BUILD_LIKE_CSEQ(x_accept_dynamic_rate, rtsp_msg_x_accept_dynamic_rate, x_accept_dynamic_rate, "x-accept-dynamic-Rate: %u")
DEFINE_PARSE_BUILD_LIKE_CSEQ(x_dynamic_rate, rtsp_msg_x_dynamic_rate, x_dynamic_rate, "x-dynamic-rate: %u")

//like Server/User-Agent char[]
...
static const rtsp_msg_str2parser_tbl_s rtsp_msg_hdr_line_parse_tbl[] = {
	...
	{15, "Authorization: ", rtsp_msg_parse_auth}, 
	{19, "WWW-Authorization: ", rtsp_msg_parse_www_auth}, 
	...
	{23, "x-accept-dynamic-rate: ", rtsp_msg_parse_x_accept_dynamic_rate}, 
	{15, "x-dynamic-rate: ", rtsp_msg_parse_x_dynamic_rate}, 
};
...
{
	if (!msg->hdrs.auth || msg->hdrs.auth->type != RTSP_MSG_AUTH_TYPE_BASIC)
		return -1;
	if (basic_b64)
...
int rtsp_msg_set_www_auth_basic (rtsp_msg_s *msg, const char *realm)
{
	...
	strncpy(msg->hdrs.www_auth->realm, realm, 
			sizeof(msg->hdrs.www_auth->realm) - 1);
	return 0;
}
...
int rtsp_msg_get_x_dynamic_rate (const rtsp_msg_s *msg, uint32_t *x_dynamic_rate)
{
	if (!msg->hdrs.x_dynamic_rate)
		return -1;
	if (x_dynamic_rate)
		*x_dynamic_rate = msg->hdrs.x_dynamic_rate->x_dynamic_rate;
	return 0;
}
